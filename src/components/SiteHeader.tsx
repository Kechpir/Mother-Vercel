...
export function SiteHeader() {
  ...
  const isToraPage = pathname === "/sessions/tora";
  const bookingHref = isToraPage
    ? TORA_REGISTER_FORM_URL
    : isProtocolPage
      ? "/sessions/energiya-pervonachalnosti#booking-form"
      : "/#register";
  const bookingLabel = isToraPage
    ? "Зарегистрироваться"
    : isProtocolPage
      ? "Приобрести запись"
      : "Забронировать место";
          ...
          <nav className="hidden md:flex flex-1 items-center justify-between min-w-0">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-9 h-9 rounded-lg text-[#25D366] hover:bg-[#25D366]/10 transition-colors duration-200"
                aria-label="WhatsApp"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
            ...
            <div className="relative" ref={sessionsDropdownRef}>
              ...
              {sessionsOpen && (
                <div
                  className="absolute top-full left-0 mt-1 py-1 min-w-[340px] max-w-[380px] rounded-xl shadow-xl z-50 border transition-colors"
                  style={{
                ...
                >
                  {SESSIONS_LINKS.map((item) => (
                    <Link
                      ...
                      className="block px-4 py-2.5 text-sm text-[#e8e0d0] hover:bg-white/5 hover:text-white transition-colors first:rounded-t-xl last:rounded-b-xl leading-snug"
                    >
                      {item.label}
                    </Link>
                  ))}
        ...
        >
          <nav className="flex flex-col py-6 px-4 gap-1">
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setMobileMenuOpen(false)}
              className="py-3 px-4 text-[#25D366] hover:text-[#25D366]/80 text-base font-medium"
            >
              WhatsApp
            </a>
            <div className="border-t border-[#e8e0d0]/20 my-2" />
