...
export function DiscountBanner() {
  ...
  return (
    <div
      className="flex items-center justify-center gap-4 md:gap-6 px-4 py-2.5 text-sm"
      style={{
        backgroundColor: "rgba(15, 14, 12, 0.98)",
        borderBottom: "1px solid rgba(212, 160, 60, 0.2)",
      }}
    >
      <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0 justify-center flex-nowrap flex-wrap">
        <span className="font-bold text-[#d4a03c] text-base md:text-lg whitespace-nowrap">
          Скидка на персональный энергетический протокол
        </span>
        <span className="text-[#e8e0d0] text-sm md:text-base whitespace-nowrap">
        ...
        <div className="flex items-center gap-1.5 md:gap-2">
          {blocks.map(({ value, label }) => (
            <div key={label} className="flex flex-col items-center">
              <div className="bg-[#d4a03c]/20 border border-[#d4a03c]/40 rounded px-2 py-1 min-w-[2.25rem] flex justify-center">
                <span className="text-[#d4a03c] font-bold text-sm tabular-nums">{value}</span>
              </div>
              <span className="text-[10px] uppercase text-[#e8e0d0]/70 mt-0.5">{label}</span>
            </div>
          ))}
        </div>
        <Link
          href="/sessions/energiya-pervonachalnosti#booking-form"
          className="px-4 py-2 rounded-xl bg-[#d4a03c] text-black font-bold text-xs uppercase tracking-wider hover:bg-[#e8b84c] transition-colors shrink-0"
        >
          Оформить
        </Link>
      </div>
