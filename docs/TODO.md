...
## После личных кабинетов

### Brevo: дублирование ссылки на Telegram в email
- [ ] Интеграция Brevo для отправки письма после успешной оплаты.
- **Уточнение:** экран после оплаты (страница success) не меняется — сообщение со ссылкой на Telegram-канал по-прежнему показывается пользователю. Добавляется только дублирование этой же ссылки на email участника (отправка письма с инвайт-ссылкой через Brevo).
